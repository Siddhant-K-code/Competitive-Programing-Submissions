...
void solve()
{
    ...
    while (q--)
    {
        ...
        s[p - 1] = c;
        if (c == '?')
            val[num[p - 1]] = val[num[p - 1] * 2] + val[num[p - 1] * 2 + 1];
        else if (c == '1')
            val[num[p - 1]] = val[num[p - 1] * 2 + 1];
        else
            val[num[p - 1]] = val[num[p - 1] * 2];
        for (int a = num[p - 1] / 2; a > 0; a /= 2)
        {
            if (s[rev[a]] == '?')
                val[a] = val[a * 2] + val[a * 2 + 1];
            else if (s[rev[a]] == '1')
                val[a] = val[a * 2 + 1];
            else
                val[a] = val[a * 2];
