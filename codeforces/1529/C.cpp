...
ll mod_dfs(ll source, ll key, ll var)
{
    ll p, ans = 0;
    // cout << dp[source][var] << endl;

    if (dp[source][var] != -1)
    {
        return dp[source][var];
    }
    // cout << var << endl;
    ...
    for (auto i : g[source])
    {
        if (i != key)
        {
            ans += max(abs(p - L[i]) + mod_dfs(i, source, 0), abs(p - R[i]) + mod_dfs(i, source, 1));
        }
    }
    // cerr << ans << endl;
    return dp[source][var] = ans;
}
...
void solve()
{
    ...
    // cout << (mod_dfs(0, -1, 0)) << " " << (mod_dfs(0, -1, 1)) << endl;

    cout << max(mod_dfs(0, -1, 0), mod_dfs(0, -1, 1)) << endl;
    // Output
