...
void solve()
{
    ...
    ll ans = 1e18;
    for (ll i = 0, j = 0; i < vo[0].size() && j < vo[1].size();)
    {
        ll ei = vo[0][i], ej = vo[1][j];

        // cerr << ei << endl;

        ans = min(ans, abs(ei - ej));
        // cout << ans << endl;
        (ei < ej ? i : j)++;
    }
    ...
    for (ll k = 0; k < 2; k++)
    {
        ll r0 = 1e18, r1 = 1e18, used_i = 0;
        for (ll i = 0, j = 0; i < ve.size() && j < vo[k].size();)
        {
        ...
        for (ll i = 0, j = 0; i < ve.size() && j < vo[1 - k].size();)
        {
